...
from .maze_functions import create_sector_toxic, create_side_def, create_spawn, gen_random_maze, create_red_pillar, create_green_pillar
...
def create_maze(base_filepath, filename, size, cell_size, keep_prob):
    ...
    exterior, walls = gen_maze(size, cell_size, xmin=xmin, ymin=ymin, keep_prob=keep_prob)
...
if __name__ == '__main__':
    ...
    BASE_FILEPATH = 'resources/scenarios/custom_scenarios/two_color_p{}/test/'
    NUM_MAZES = 64
    
    for p in range(5,8):
        for m in range(NUM_MAZES):
            filename = 'two_color_maze{:003}.wad'.format(m)
            print('creating maze', filename)
        
            create_maze(BASE_FILEPATH.format(p), filename, size=5, 
                        cell_size=256, keep_prob=p)
